//! Rust Code Generator
//!
//! This module provides the main code generator for converting PowerPC instructions
//! to Rust code. It includes optimizations and support for various instruction types.
...
pub struct CodeGenerator {
    ...
    register_types: HashMap<u8, crate::recompiler::analysis::TypeInfo>, // Register type information
}
...
impl CodeGenerator {
    ...
    /// Set register types from type inference
    pub fn set_register_types(&mut self, types: HashMap<u8, crate::recompiler::analysis::TypeInfo>) {
    ...
    /// Generate a complete function file with imports and function code
    ///
    /// # Arguments
    /// * `metadata` - Function metadata
    /// * `instructions` - Instructions for the function
    /// * `module_path` - Optional module path for imports
    ///
    /// # Returns
    /// `Result<String>` - Complete function file content
    pub fn generate_function_file(
        ...
        // Add file header
        code.push_str("//! Auto-generated recompiled function\n");
        code.push_str("//! Generated by GCRecomp\n\n");

        // Add imports
        code.push_str("use crate::runtime::context::CpuContext;\n");
        code.push_str("use crate::runtime::memory::MemoryManager;\n");
        code.push_str("use anyhow::Result;\n\n");

        // Add function dispatcher import if needed
        if self.has_function_calls(instructions) {
            if let Some(module) = module_path {
                // Calculate relative path to root
                let depth = module.matches('/').count();
                let prefix = "../".repeat(depth);
                code.push_str(&format!(
                    "use {}recompiled::call_function_by_address;\n\n",
                    prefix
                ));
            } else {
                code.push_str("use super::call_function_by_address;\n\n");
            }
        }
    ...
    /// Generate function code (for backward compatibility)
    pub fn generate_function(
        &mut self,
        metadata: &FunctionMetadata,
        instructions: &[DecodedInstruction],
    ) -> Result<String> {
        self.generate_function_file(metadata, instructions, None)
    }

    /// Check if function contains function calls
    fn has_function_calls(&self, instructions: &[DecodedInstruction]) -> bool {
        instructions.iter().any(|inst| {
            matches!(inst.instruction.instruction_type, InstructionType::Branch)
                && inst.instruction.opcode == 18
                && (inst.raw & 1) != 0 // Link bit set (function call)
        })
    }
            ...
            for (inst_idx, instruction) in block.iter().enumerate() {
                match self.generate_instruction(instruction) {
                    ...
                    Err(e) => {
                        ...
                        code.push_str(&self.indent());
                        code.push_str(&format!(
                            "// Opcode 0x{:02X} - using generic instruction handler\n",
    ...
    fn build_basic_blocks(
        &self,
        instructions: &[DecodedInstruction],
    ) -> Result<Vec<Vec<&DecodedInstruction>>> {
    ...
    fn generate_arithmetic(
        &mut self,
        inst: &crate::recompiler::decoder::Instruction,
    ) -> Result<String> {
        let mut code = String::new();
        ...
        // Use typed register access for better code generation
        let ra_typed = self.get_typed_register(ra_reg);
        
        // Handle shift operations specially
        let operation_code = if op == "<<" || op == ">>" {
            if op == "<<" {
                format!("{} << ({} & 0x1F)", ra_typed, rb_expr)
            } else {
                format!("{} >> ({} & 0x1F)", ra_typed, rb_expr)
            }
        } else {
            // Use typed operations to avoid unnecessary conversions
            let rb_typed = if let Some(RegisterValue::Variable(_)) = rb_value {
                // If rb is a register, use typed access
                if let Operand::Register(rb_reg) = &inst.operands[2] {
                    self.get_typed_register(*rb_reg)
                } else {
                    rb_expr
                }
            } else {
    ...
    fn generate_load(&mut self, inst: &crate::recompiler::decoder::Instruction) -> Result<String> {
        ...
        if let Some(RegisterValue::Constant(base)) = base_value {
            ...
            // Use typed register set to avoid unnecessary conversions
            code.push_str(&self.indent());
            code.push_str(&format!("{};\n", self.set_typed_register(rt_reg, "value")));
            self.set_register_value(rt_reg, RegisterValue::Unknown);
    ...
    fn generate_branch(
        ...
        match inst.operands.len() {
            ...
            3..=5 => {
                ...
                code.push_str(&self.indent());
                code.push_str(&format!("if cr_bit != 0 {{\n"));
                self.indent_level += 1;
                code.push_str(&self.indent());
                code.push_str(&format!(
    ...
    fn generate_condition_register(
        &mut self,
        inst: &crate::recompiler::decoder::Instruction,
    ) -> Result<String> {
        let mut code = String::new();
    ...
    fn generate_generic(&mut self, inst: &DecodedInstruction) -> Result<String> {
        ...
        code.push_str(&self.indent());
        code.push_str(&format!(
            "// Unimplemented instruction: type={:?}, opcode=0x{:02X}, raw=0x{:08X}\n",
            inst.instruction.instruction_type, inst.opcode, inst.raw
        ));
        code.push_str(&self.indent());
        code.push_str("log::warn!(\"Unimplemented instruction at 0x{:08X}: type={:?}, opcode=0x{:02X}\", ");
        code.push_str(&format!("0x{:08X}, ", inst.address));
        code.push_str(&format!("{:?}, ", inst.instruction.instruction_type));
        code.push_str(&format!("0x{:02X});\n", inst.opcode));
        
        // Generate safe fallback: call runtime function for unimplemented instructions
        code.push_str(&self.indent());
        code.push_str("// Call runtime handler for unimplemented instruction\n");
        code.push_str(&self.indent());
        code.push_str("if let Err(e) = crate::runtime::sdk::handle_unimplemented_instruction(");
        code.push_str(&format!("0x{:08X}, ", inst.address));
        code.push_str(&format!("0x{:08X}", inst.raw));
        code.push_str(", ctx, memory) {\n");
        self.indent_level += 1;
        code.push_str(&self.indent());
        code.push_str("log::error!(\"Error handling unimplemented instruction: {}\", e);\n");
        code.push_str(&self.indent());
        code.push_str("return Err(anyhow::anyhow!(\"Unimplemented instruction failed\"));\n");
        self.indent_level -= 1;
        code.push_str(&self.indent());
        code.push_str("}\n");
    ...
    /// Get typed register access code
    /// Returns code that reads a register with the appropriate type
    fn get_typed_register(&mut self, reg: u8) -> String {
        if let Some(ty) = self.register_types.get(&reg) {
            let rust_type = self.type_to_rust(ty);
            match ty {
                crate::recompiler::analysis::TypeInfo::Integer { signed: true, size: 32 } => {
                    format!("ctx.get_register({}) as i32", reg)
                }
                crate::recompiler::analysis::TypeInfo::Integer { signed: false, size: 32 } => {
                    format!("ctx.get_register({})", reg)
                }
                crate::recompiler::analysis::TypeInfo::Float { size: 32 } => {
                    format!("f32::from_bits(ctx.get_register({}))", reg)
                }
                crate::recompiler::analysis::TypeInfo::Float { size: 64 } => {
                    format!("f64::from_bits((ctx.get_register({}) as u64) | ((ctx.get_register({}) as u64) << 32))", reg, reg + 1)
                }
                crate::recompiler::analysis::TypeInfo::Pointer { .. } => {
                    format!("ctx.get_register({}) as *mut u8", reg)
                }
                _ => {
                    format!("ctx.get_register({})", reg)
                }
    ...
    /// Set typed register value
    fn set_typed_register(&mut self, reg: u8, value: &str) -> String {
        if let Some(ty) = self.register_types.get(&reg) {
            match ty {
                crate::recompiler::analysis::TypeInfo::Float { size: 32 } => {
                    format!("ctx.set_register({}, {} as u32)", reg, value)
                }
                crate::recompiler::analysis::TypeInfo::Float { size: 64 } => {
                    format!("ctx.set_register({}, {} as u32); ctx.set_register({}, ({} as u64 >> 32) as u32)", 
                           reg, value, reg + 1, value)
                }
                crate::recompiler::analysis::TypeInfo::Pointer { .. } => {
                    format!("ctx.set_register({}, {} as u32)", reg, value)
                }
    ...
    fn type_to_rust(&self, ty: &crate::recompiler::analysis::TypeInfo) -> String {
        match ty {
            ...
            crate::recompiler::analysis::TypeInfo::Float { size } => {
                match *size {
                    32 => "f32".to_string(),
                    64 => "f64".to_string(),
                    _ => "f32".to_string(),
            ...
            crate::recompiler::analysis::TypeInfo::Struct { name, .. } => {
                format!("{}", self.sanitize_identifier(name))
            }
            _ => "u32".to_string(), // Default to u32 for unknown types
        }
    }
